...
const nextTagIndex = pageContent.indexOf("                  </>", startIndex);

if (startIndex !== -1 && nextTagIndex !== -1) {
  // Find the exact `                    )}` just before nextTagIndex
  const chunk = pageContent.substring(startIndex, nextTagIndex);
  const localEndIndex = chunk.lastIndexOf(")}");
  
  if (localEndIndex !== -1) {
      endIndex = startIndex + localEndIndex + 2;
...
`;
      ...
      pageContent = pageContent.substring(0, startIndex) + newStaticBlock + pageContent.substring(endIndex);
      fs.writeFileSync(pagePath, pageContent);
      console.log("Successfully replaced the inline logic!");
  } else {
      console.log("Could not find local end index.");
  }
} else {
  console.log("Could not find start or nextTag pattern.");
}
